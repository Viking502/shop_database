...
AS
    WITH finalized_orders_ids AS
    (
        SELECT DISTINCT order_id
        FROM OrderHistory
        WHERE status = 'finalized'
    ),
    not_finalized_orders_last_ids AS
    (
    ...
    SELECT not_finalized_orders_last_ids.order_id, CONCAT(U.name, ' ', U.surname) as ClientName,
           H.status, O.date AS SinceWhen
